...
export function MockServerManager() {
  ...
  const toggleServer = useCallback((id: string) => {
    saveServers(servers.map((s) => 
      s.id === id ? { ...s, enabled: !s.enabled } : s
    ));
  }, [servers]);

  const updateServerName = useCallback((id: string, name: string) => {
    saveServers(servers.map((s) => 
      s.id === id ? { ...s, name } : s
    ));
  }, [servers]);
  ...
  const updateEndpoint = useCallback((endpointId: string, updates: Partial<MockEndpoint>) => {
    saveServers(servers.map((s) => 
      s.id === selectedServerId 
        ? {
            ...s,
          ...
          <>
            <div className="p-3 border-b border-border flex items-center gap-2">
              <input
                ...
                onChange={(e) => updateServerName(selectedServer.id, e.target.value)}
                    ...
                    {selectedServer.endpoints.map((endpoint) => (
                      <div
                        ...
                        className={`p-3 border rounded-lg ${
                          endpoint.enabled ? "bg-card" : "bg-muted/50 opacity-60"
                        }`}
                      >
            ...
            {editingEndpoint && (
              <div className="border-t border-border p-3 bg-card max-h-96 overflow-auto">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateEndpoint(editingEndpoint.id, { name: e.target.value })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateEndpoint(editingEndpoint.id, { path: e.target.value })}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateEndpoint(editingEndpoint.id, { method: e.target.value })}
                      className="w-full px-2 py-1.5 text-xs bg-input border border-border rounded"
                    >
                      <option>GET</option>
                      <option>POST</option>
                  ...
                  <div>
                    <label className="block text-xs text-muted-foreground mb-1">Status Code</label>
                    <input
                      type="number"
                      value={editingEndpoint.statusCode}
                      onChange={(e) => updateEndpoint(editingEndpoint.id, { statusCode: parseInt(e.target.value) || 200 })}
                      className="w-full px-2 py-1.5 text-xs bg-input border border-border rounded"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateEndpoint(editingEndpoint.id, { contentType: e.target.value })}
                      className="w-full px-2 py-1.5 text-xs bg-input border border-border rounded"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateEndpoint(editingEndpoint.id, { delay: parseInt(e.target.value) || 0 })}
                      className="w-full px-2 py-1.5 text-xs bg-input border border-border rounded"
                    />
                  </div>
                </div>
                ...
                <div className="mt-3">
                  <label className="block text-xs text-muted-foreground mb-1">Response Body</label>
                  <textarea
                    value={editingEndpoint.body}
                    onChange={(e) => updateEndpoint(editingEndpoint.id, { body: e.target.value })}
                    rows={6}
                    className="w-full px-2 py-1.5 text-xs bg-input border border-border rounded font-mono"
                  />
                </div>

...
export function HeadersEditor({ headers, onChange }: HeadersEditorProps) {
  ...
  const inputRefs = useRef<
    Array<{ name: HTMLInputElement | null; value: HTMLInputElement | null }>
  >([]);
        ...
        {bulkMode ? (
          <div className="p-3">
            ...
            <div className="mt-2 text-[10px] text-muted-foreground">
              One header per line:{" "}
              <code className="px-1 bg-muted rounded">Name: Value</code>
            </div>
          </div>
        ) : headers.length === 0 ? (
                        ...
                        if (suggestions.length > 0 && header.value) {
                          setAutocompleteOptions(
                            suggestions.filter((s) =>
                              s
                                .toLowerCase()
                                .includes(header.value.toLowerCase()),
                            ),
                          );
                          setActiveAutocomplete(index);
                        }

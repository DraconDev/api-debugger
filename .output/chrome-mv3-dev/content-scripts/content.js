var content = (function() {
  ...
  function useAuth(options) {
    const { authFlow, authStore, getUser } = options;
        ...
        if (newState.isAuthenticated && getUser) {
          try {
            ...
            if (userData?.subscription) {
              newState.subscription = userData.subscription;
    ...
    reactExports.useEffect(() => {
      ...
      const listener = (changes) => {
        if ("local:auth" in changes || "auth" in changes) {
          refresh();
        }
      };
      browser$1.storage.onChanged.addListener(listener);
      return () => browser$1.storage.onChanged.removeListener(listener);
    }, [refresh]);
  ...
  function createConfig(options) {
    let env = options.env;
    if (!env && options.autoDetectEnv !== false) {
      env = detectEnvironment();
    }
    env = env || __vite_import_meta_env__?.VITE_APP_ENV || "local";
  ...
  function createApiClient(options) {
    ...
    async function request(endpoint, opts = {}) {
      const isContentScript2 = typeof window !== "undefined" && (window.location.protocol === "http:" || window.location.protocol === "https:");
      if (isContentScript2) {
        try {
          const response = await browser$1.runtime.sendMessage({
  ...
  function createAuthFlow(options) {
    const { config, setAuth, resetAuth, appId } = options;
    function getLoginUrl() {
      const redirectUrl = browser$1.runtime.getURL("auth-callback.html");
      return `${config.draconUrl}/api/v1/auth/login/google?redirect_uri=${encodeURIComponent(redirectUrl)}&app=${appId}`;
    }
    function openLogin() {
      browser$1.tabs.create({ url: getLoginUrl() });
    }
    function openDashboard() {
      browser$1.tabs.create({ url: `${config.draconUrl}/dashboard/launch` });
    }
  ...
  const browser = globalThis.browser?.runtime?.id ? globalThis.browser : globalThis.chrome;
  ...
  const definition = defineContentScript({
    ...
    main() {
      ...
      browser$2.runtime.onMessage.addListener((message, sender, sendResponse) => {
        console.log("[Content] Received message:", message);
        if (message.type === "ping") {
          sendResponse({ pong: true, url: window.location.href });
        }
        if (message.type === "getPageContent") {
          const content2 = document.body.innerText;
          sendResponse({ content: content2 });
        }
        return true;
      });
    }
  ...
  function getUniqueEventName(eventName) {
    return `${browser$2?.runtime?.id}:${"content"}:${eventName}`;
  }
  ...
  var ContentScriptContext = class ContentScriptContext2 {
    ...
    get isInvalid() {
      if (browser$2.runtime?.id == null) this.notifyInvalidated();

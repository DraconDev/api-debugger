...
chrome.storage.local.get(["apiDebugger_mockServers"]).then((result) => {
  if (result.apiDebugger_mockServers) {
    mockServers = result.apiDebugger_mockServers;
  }
});
    ...
    for (const server of mockServers) {
      ...
      for (const endpoint of server.endpoints) {
        ...
        if (details.method === endpoint.method && url.pathname === endpoint.path) {
          const responseHeaders: chrome.webRequest.HttpHeader[] = Object.entries(endpoint.headers).map(
            ([name, value]) => ({ name, value })
          );
          
          const encoder = new TextEncoder();
          const body = encoder.encode(endpoint.body);
          
          setTimeout(() => {
            chrome.webRequest.onCompleted.addListener(
              function mockHandler(completedDetails: chrome.webRequest.WebResponseCacheDetails) {
                if (completedDetails.requestId === details.requestId) {
                  chrome.webRequest.onCompleted.removeListener(mockHandler);
                }
              },
              { urls: ["<all_urls>"] }
            );
